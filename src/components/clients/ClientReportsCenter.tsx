...
export const ClientReportsCenter = ({ className = '' }: ClientReportsCenterProps) => {
  ...
  // Estados locais
  const [tipoRelatorio, setTipoRelatorio] = useState<ConfigRelatorioCliente['tipo']>('PERFIL')
  const [periodo, setPeriodo] = useState<DateRange>(getMonthRange())
  const [filtros, setFiltros] = useState<ConfigRelatorioCliente['filtros']>({})
  const [formatoExportacao, setFormatoExportacao] =
    useState<ConfigRelatorioCliente['formato']>('CSV')
  ...
  const tiposRelatorio = [
    {
      ...
      descricao: 'Análise completa do perfil dos clientes',
      icon: Users,
      color: 'text-blue-600',
    },
    {
      ...
      nome: 'Frequência',
      descricao: 'Análise de frequência de visitas',
      icon: TrendingUp,
      color: 'text-green-600',
    },
    {
      tipo: 'FIDELIZACAO' as const,
      nome: 'Fidelização',
      descricao: 'Análise de fidelidade e retenção',
      icon: Heart,
      color: 'text-red-600',
    },
    {
      tipo: 'SEGMENTACAO' as const,
      nome: 'Segmentação',
      descricao: 'Segmentação de clientes por perfil',
      icon: Target,
  ...
  const handleGerarRelatorio = async () => {
    const config: ConfigRelatorioCliente = {
      tipo: tipoRelatorio,
      periodo,
      filtros,
    ...
    switch (tipoRelatorio) {
      case 'PERFIL':
        await gerarRelatorioClientes(config)
        break
      case 'FREQUENCIA':
        await gerarRelatorioFrequencia(config)
        break
      case 'FIDELIZACAO':
        await gerarRelatorioFidelizacao(config)
        break
      case 'SEGMENTACAO':
        await gerarRelatorioSegmentacao(config)
        break
    }
  }
  ...
  const getDadosRelatorioAtual = () => {
    switch (tipoRelatorio) {
      ...
      case 'FIDELIZACAO':
        return relatorioFidelizacao
      case 'SEGMENTACAO':
        return relatorioSegmentacao
      default:
        return null
    }
  }
        ...
        <div className="mb-6 flex items-center justify-center space-x-4">
          ...
          <div>
            ...
            <p className="text-lg font-medium text-gray-600 dark:text-gray-300">
              Análise completa do perfil, frequência e fidelização dos clientes
            </p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="roberto">Roberto Lima</option>
              </select>

              <select
                value={filtros.statusFidelidade || ''}
                onChange={(e) =>
                  setFiltros((prev) => ({ ...prev, statusFidelidade: e.target.value || undefined }))
                }
                className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-gold dark:border-secondary-graphite-card/30 dark:bg-secondary-graphite-light dark:text-white"
              >
                  ...
                  {Object.entries(dadosAtual.resumo).map(([key, value]) => (
                    ...
                    >
                      ...
                      <div className="font-bold text-gray-900 dark:text-white">
                        {typeof value === 'number' &&
                        (key.includes('valor') || key.includes('Valor'))
      ...
      {!isLoading && !dadosAtual && !isError && (
        <Card className="rounded-xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-12 text-center shadow-lg dark:border-secondary-graphite dark:from-secondary-graphite-light dark:to-secondary-graphite">
          <div className="mx-auto mb-6 w-fit rounded-2xl bg-primary-gold/10 p-6">
            <Users className="mx-auto h-16 w-16 text-primary-gold" />
          </div>
          ...
          <p className="mb-6 font-medium text-gray-600 dark:text-gray-300">
            ...
            Relatório&quot; para visualizar os dados dos clientes.
          </p>
          <Button
            onClick={handleGerarRelatorio}
            disabled={isLoading}
