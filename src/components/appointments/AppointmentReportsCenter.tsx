...
import {
  ...
  type DateRange,
} from '@/hooks/use-appointment-reports'
import { getMonthRange, formatCurrency } from '@/components/financial/utils'

interface AppointmentReportsCenterProps {
  className?: string
}

export const AppointmentReportsCenter = ({ className = '' }: AppointmentReportsCenterProps) => {
  ...
  // Estados locais
  const [tipoRelatorio, setTipoRelatorio] = useState<ConfigRelatorioAgendamento['tipo']>('OCUPACAO')
  const [periodo, setPeriodo] = useState<DateRange>(getMonthRange())
  const [filtros, setFiltros] = useState<ConfigRelatorioAgendamento['filtros']>({})
  const [formatoExportacao, setFormatoExportacao] =
    useState<ConfigRelatorioAgendamento['formato']>('CSV')

  // Tipos de relatório disponíveis
  const tiposRelatorio = [
    {
      tipo: 'OCUPACAO' as const,
      nome: 'Taxa de Ocupação',
      descricao: 'Análise de ocupação por barbeiro',
      icon: Activity,
    ...
    },
    {
      ...
      nome: 'Performance',
      descricao: 'Performance e eficiência dos barbeiros',
      icon: TrendingUp,
      color: 'text-blue-600',
    },
    {
      tipo: 'HORARIOS' as const,
      nome: 'Horários',
      descricao: 'Análise de horários mais procurados',
      icon: Clock,
      color: 'text-purple-600',
    },
    {
      tipo: 'CANCELAMENTOS' as const,
      nome: 'Cancelamentos',
      descricao: 'Análise de cancelamentos e no-shows',
      icon: AlertTriangle,
      color: 'text-red-600',
    },
  ]
  ...
  const handleGerarRelatorio = async () => {
    const config: ConfigRelatorioAgendamento = {
      tipo: tipoRelatorio,
      periodo,
      filtros,
    ...
    switch (tipoRelatorio) {
      case 'OCUPACAO':
        await gerarRelatorioOcupacao(config)
        break
      case 'PERFORMANCE':
        await gerarRelatorioPerformance(config)
        break
      case 'HORARIOS':
        await gerarRelatorioHorarios(config)
        break
      case 'CANCELAMENTOS':
        await gerarRelatorioCancelamentos(config)
        break
    }
  }
        ...
        <div className="mb-6 flex items-center justify-center space-x-4">
          ...
          <div>
            ...
            <p className="text-lg font-medium text-gray-600 dark:text-gray-300">
              Análise de ocupação, performance e estatísticas de agendamentos
            </p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="3">Corte Premium</option>
              </select>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setFiltros((prev) => ({
                        ...prev,
                        horarioInicio: e.target.value || undefined,
                      }))
                    }
                    className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-gold dark:border-secondary-graphite-card/30 dark:bg-secondary-graphite-light dark:text-white"
                  />
                  <input
                    type="time"
                    ...
                    onChange={(e) =>
                      setFiltros((prev) => ({ ...prev, horarioFim: e.target.value || undefined }))
                    }
                    className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-gold dark:border-secondary-graphite-card/30 dark:bg-secondary-graphite-light dark:text-white"
                  />
                </div>
      ...
      {!isLoading && !dadosAtual && !isError && (
        <Card className="rounded-xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 p-12 text-center shadow-lg dark:border-secondary-graphite dark:from-secondary-graphite-light dark:to-secondary-graphite">
          <div className="mx-auto mb-6 w-fit rounded-2xl bg-primary-gold/10 p-6">
            <Calendar className="mx-auto h-16 w-16 text-primary-gold" />
          </div>
          ...
          <p className="mb-6 font-medium text-gray-600 dark:text-gray-300">
            ...
            Relatório&quot; para visualizar os dados de agendamentos.
          </p>
          <Button
            onClick={handleGerarRelatorio}
            disabled={isLoading}
